...
void *updatePheromone(void *arg)
{
    int threadId = *(int *)arg;
    int start = threadId * (MAX_NODES / NUM_THREAD);
    int end = min((threadId + 1) * (MAX_NODES / NUM_THREAD), MAX_NODES);
    int from, to, i, ant;
    ...
    start = threadId * (MAX_ANTS / NUM_THREAD);
    end = min((threadId + 1) * (MAX_ANTS / NUM_THREAD), MAX_ANTS);
...
void *acoSolve(void *arg)
{
    int threadId = *(int *)arg;
    int start = threadId * (MAX_ANTS / NUM_THREAD);
    int end = min((threadId + 1) * (MAX_ANTS / NUM_THREAD), MAX_ANTS);
...
int main()
{
    ...
    for (int i = 0; i < MAX_ITERATIONS; i++)
    {
        ...
        {
            thread_ids[i] = i;
            pthread_create(&threads[i], NULL, acoSolve, (void *)&thread_ids[i]);
        }
        for (int i = 0; i < NUM_THREAD; i++)
        {
        ...
        {
            thread_ids[i] = i;
            pthread_create(&threads[i], NULL, updatePheromone, (void *)&thread_ids[i]);
        }
        for (int i = 0; i < NUM_THREAD; i++)
        {
            pthread_join(threads[i], NULL);
